...
if (isset($_POST['btn_add'])) {
    ...
    if (empty($error)) {
        $sql = "INSERT INTO `info` (`name` , `phone`, `img`, `detail`,`email`,`local`,`user_add`)" . "VALUES ('{$res_name}','{$res_number}','{$upload_file}','{$res_des}','{$res_email}','{$res_local}','{$user_login}')";
...
<body class="d-flex flex-column min-vh-100" style="min-height: 600px;">
    ...
    <?php if (empty($error) && isset($_POST['btn_add'])) {
                        echo "<div class='alert alert-success alert-dismissible fade show position-fixed w-100' role='alert'>
                        <strong>Thanh cong</strong> Ban da them thanh cong!
                        <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
                    </div>";
                    }
                    ?>
    <!-- End navigation -->
    <div class="container" style="background-color: white; margin-top: 70px; border-radius: 16px;">
