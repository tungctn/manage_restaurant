...
if (isset($_POST['btn_update'])) {
    ...
    // echo $id_;
    $sql = "UPDATE `info` SET `name` = '$res_name', `email` = '$res_email', `phone` = '$res_number', `detail` = '$res_des' WHERE `info`.`id` = {$id}";
...
$sql = "SELECT * FROM `info` WHERE `info`.`id` =  {$id}";
...
<body class="d-flex flex-column min-vh-100" style="min-height: 600px;">
    ...
    <?php if (empty($error) && isset($_POST['btn_update'])) echo "<div class='alert alert-success alert-dismissible fade show position-fixed w-100' role='alert'>
